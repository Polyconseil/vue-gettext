...
describe("Translate tests", () => {
  ...
  it("tests the ngettext() method", () => {
    ...
    // If no translation exists, display the default singular form (if n < 2).
    Vue.config.language = "fr_FR";
    expect(undetectableNgettext("Untranslated %{ n } item", "Untranslated %{ n } items", -1)).to.equal(
      "Untranslated %{ n } item"
    );

    // If no translation exists, display the default plural form (if n > 1).
    Vue.config.language = "fr_FR";
    expect(undetectableNgettext("Untranslated %{ n } item", "Untranslated %{ n } items", 2)).to.equal(
  ...
  it("tests the npgettext() method", () => {
    ...
    Vue.config.language = "fr_FR";
    expect(undetectableNpgettext("Noun", "%{ carCount } car (noun)", "%{ carCount } cars (noun)", 2)).to.equal(
      "%{ carCount } véhicules (nom)"
    );

    Vue.config.language = "en_US";
    expect(undetectableNpgettext("Verb", "%{ carCount } car (verb)", "%{ carCount } cars (verb)", 2)).to.equal(
      "%{ carCount } cars (verb)"
    );

    Vue.config.language = "fr_FR";
    expect(undetectableNpgettext("Noun", "%{ carCount } car (noun)", "%{ carCount } cars (noun)", 1)).to.equal(
      "%{ carCount } véhicule (nom)"
    );

    Vue.config.language = "en_US";
    expect(undetectableNpgettext("Verb", "%{ carCount } car (verb)", "%{ carCount } cars (verb)", 1)).to.equal(
      "%{ carCount } car (verb)"
    );
