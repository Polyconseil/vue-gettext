...
describe('GetText plugin configuration tests', () => {
  ...
  it('raises an error when an unknown option is used', () => {
    ...
    }).toThrowError('unknownOption is an invalid option for the translate plugin.')
  })

  it('raises an error when there are no translations', () => {
    expect(function () {
      Vue.use(GetTextPlugin, {})
    }).toThrowError('No translations available.')
  })
  ...
  it('allows to add a mixin to languageVm', () => {
    ...
    expect(vm.$language.currentKebabCase).toBe('fr-fr')
    vm.$language.current = 'en_GB'
    expect(vm.$language.currentKebabCase).toBe('en-gb')
  })
